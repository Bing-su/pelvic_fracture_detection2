    ...
    def init_model_ref(self):
        ...
        for var in list(self.model.parameters()) + [self.a, self.b]:
            self.model_ref.append(
                torch.empty(var.shape).normal_(mean=0, std=0.01).to(self.device)
            )
        ...
        for group in self.param_groups:
            ...
            for i, p in enumerate(group["params"]):
                ...
                p.data = p.data - group["lr"] * (
                    ...
                    + 1 / gamma * (p.data - model_ref[i].data)
                    + weight_decay * p.data
                )
                model_acc[i].data = model_acc[i].data + p.data

            alpha.data = alpha.data + group["lr"] * (
                2 * (m + b.data - a.data) - 2 * alpha.data
            )
    ...
    def update_regularizer(self, decay_factor=None):
        if decay_factor is not None:
            ...
            print(
                ...
                % (self.param_groups[0]["lr"], self.T)
            )
        print("Updating regularizer @ T=%s!" % (self.T))
        for i, _ in enumerate(self.model_ref):
